  ...
  sidebarLayout(
    sidebarPanel(
      ...
      conditionalPanel(
        ...
        selectInput(
          "minNmax","chose one:",
          c("null","minimum temperature","maximum temperature")),
        conditionalPanel(
          condition="input.minNmax=='minimum temperature'",
          checkboxInput("tempMin", "add moving average line?",FALSE),
          conditionalPanel(
            condition='input.tempMin == true',
            sliderInput("minSlide",
                        "enter a bandwidth:",
                        min = 1,
                        max = 20,
                        value = 5))),
        conditionalPanel(
          condition="input.minNmax=='maximum temperature'",
          checkboxInput("tempMax", "add moving average line?",FALSE),
          conditionalPanel(
            condition='input.tempMax == true',
            sliderInput("maxSlide",
                        "enter a bandwidth:",
                        min = 1,
                        max = 20,
                        value = 5)))
